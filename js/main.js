	  ...
	  success: function(xml)
			   {
					...
					xmldoc.find("item").each(function()
					{
						var xmlitem = $(this);
						var item = new Object();
						item.category = $.trim(xmlitem.find("category").text());
						item.name = $.trim(xmlitem.find("name").text());
						item.quantity = $.trim(xmlitem.find("quantity").text());
						item.idno = $.trim(xmlitem.find("idno").text());
						item.date = $.trim(xmlitem.find("date").text());
						item.comments = $.trim(xmlitem.find("comments").text());
						item.important = $.trim(xmlitem.find("important").text());
						its.push(item);
					});
			}, 
	  error: function(){alert("Error in loading data from xml!!!");}   
	 });
	 return its;
}


var loadFromCSV = function()
{
	var its = new Array();
	$.ajax({
	  url: 'data/data.csv',    
